		...
		<div class="default-node" style="height: 112px">
			...
			<div class="details-row">
				...
				<span class="status-content" style="padding-right: 0">
					...
					>
						<q-tooltip v-model="showLog" no-parent-event max-width="500px" class="bg-red-2 text-negative" style="pointer-events: all !important">
							<div class="text-subtitle1 text-weight-medium row" style="user-select: none">
								错误日志
								<q-space />
								<q-icon color="negative" name="content_copy" size="16px" @click="copyText(data.log)" class="cursor-pointer q-mr-xs" style="height: 28px" />
								<q-icon color="negative" name="close" size="20px" @click="showLog = false" class="cursor-pointer" style="height: 28px" />
							</div>
							<div>{{ data.log }}</div>
						</q-tooltip>
					</q-icon>
...
import NodeState from '../../InstanceNodeState.vue';
import { InstanceNodeState } from 'src/service/instance';
import { useQuasar, copyToClipboard } from 'quasar';

const NodeType = [{ name: 'SoftwareUsecaseComputing', label: '高性能计算' }];
