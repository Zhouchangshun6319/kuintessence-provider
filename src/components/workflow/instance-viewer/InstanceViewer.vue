...
const updateStatus = () => {
	...
	props.nodeList?.forEach((newNode) => {
...
function initGraph() {
	...
	graph.value = new Graph({
		container: document.getElementById('viewer-container'), // 画布容器
		background: {
			opacity: 0,
		},
		autoResize: true,
...
function initEvent() {
	...
	graph.value.on('blank:contextmenu', () => {
	...
	graph.value.on('scale', () => {
		setTimeout(() => {
			zoom.value = Number((graph.value.zoom() * 100).toFixed(0));
		}, 50);
	});
	// 绑定节点数据更改事件
