			...
			<q-card square flat>
				<q-card-section class="q-py-sm">
					<div class="row" style="height: 40px; line-height: 40px">
						<q-input rounded outlined dense debounce="300" v-model="filter" placeholder="搜索黑名单名称" style="width: 200px">
						...
						<q-btn color="primary" label="批量删除" class="q-ml-md" :disable="selected.length === 0 || loadingBatch" :loading="loadingBatch">
							<confirm-popup icon="error" label="确定删除选中的黑名单记录吗？" @onOk="deleteBatch" />
						</q-btn>
					</div>
				</q-card-section>
			</q-card>
			...
			>
				...
				<template v-slot:body="props">
					<q-tr :props="props">
						...
						<q-td key="last_modified_time" :props="props">
							{{ date.formatDate(props.row.last_modified_time, 'YYYY/MM/DD HH:mm:ss') }}
						</q-td>
						<q-td key="function" :props="props" auto-width class="text-grey">
							...
							<q-btn dense flat color="primary" label="删除">
								<confirm-popup icon="error" label="确定删除这个黑名单记录吗？" @onOk="deleteById(props.row.id)" />
							</q-btn>
						</q-td>
					</q-tr>
				</template>
...
import { SoftwareBlackList, SoftwareBlackListService } from 'src/service/software-black-list';
import ConfirmPopup from '../..//components/common/ConfirmPopup.vue';
import FormDialog from '../../components/manage/software-black-list/FormDialog.vue';
...
const columns = [
	...
	{ name: 'created_time', label: '创建时间', field: 'created_time', align: 'left', sortable: true },
	{
 ...
 * 软件黑名单列表
 */
const rows = ref<SoftwareBlackList[]>([]);
/**
 * 添加&编辑面板
 */
...
const deleteById = async (id: string) => {
	const result = await SoftwareBlackListService.deleteById(id);
	if (result) {
		$q.notify({
			type: 'positive',
			message: '删除成功',
...
const deleteBatch = async () => {
	...
	for (let i = 0; i < selected.value.length; i++) {
		const result = await SoftwareBlackListService.deleteById(selected.value[i]);
		if (!result) {
			// 执行过程中出错
			$q.notify({
				type: 'negative',
...
const onRequest = async (props: any) => {
	...
	try {
		// 获取列表大小并更新rowsNumber
		pagination.value.rowsNumber = await SoftwareBlackListService.getListNumber(filter);
		// 获取列表并更新rows
		rows.value = await SoftwareBlackListService.getList(page, rowsPerPage !== 0 ? rowsPerPage : pagination.value.rowsNumber, filter, sortBy, descending);
	} catch (e) {
		$q.notify({
			type: 'negative',
			message: String(e),
