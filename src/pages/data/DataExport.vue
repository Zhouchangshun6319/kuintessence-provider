			...
			>
				<template v-slot:body="props">
					<q-tr :props="props">
						...
						<q-td key="toServerName" :props="props">
							{{ props.row.toServerName }}
						</q-td>
						...
						<q-td key="oprate" :props="props">
							<q-btn class="table-btn" label="去处理" @click="toDetail(props.row.id)"></q-btn>
						</q-td>
					</q-tr>
				</template>
			</q-table>
...
import { state, priority, DataExportService } from '../../service/data-export';
...
export default defineComponent({
	...
	setup() {
		...
		function toDetail(id: string) {
			router.push('/data/data-export-detail/' + id);
		}
		//列表数据
		const columns = [
			{
		...
		async function getList() {
			let list = await DataExportService.getList();
			rows.length = 0;
			rows.push(...list);
		}
		getList();
